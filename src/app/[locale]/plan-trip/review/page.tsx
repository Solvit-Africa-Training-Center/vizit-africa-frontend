    ...
    return (
      <div className="min-h-screen bg-background pt-32 pb-20 flex items-center justify-center px-6">
        <div className="max-w-md w-full text-center space-y-6">
            ...
            <h1 className="text-3xl font-display font-medium">Request Received!</h1>
            <p className="text-muted-foreground">
                Thank you, {tripInfo.name}. We have received your trip request for {items.length} items. 
                Our team will review your itinerary and get back to you shortly at {tripInfo.email}.
            </p>
            <div className="pt-8">
                <Link href="/">
      ...
      <div className="max-w-7xl mx-auto px-5 md:px-10 grid lg:grid-cols-12 gap-12">
            ...
            <div className="space-y-4">
                <AnimatePresence mode="popLayout">
                    {items.length === 0 ? (
                        ...
                        items.map((item) => (
                            <TripItemCard key={item.id} item={item} onRemove={() => removeItem(item.id)} />
                        ))
                    )}
                </AnimatePresence>
            </div>
        ...
        <div className="lg:col-span-5 space-y-8">
            <div className="bg-muted/30 border border-border/50 rounded-2xl p-6 md:p-8 sticky top-24">
                <h3 className="font-display font-medium text-lg mb-6 uppercase tracking-wider text-muted-foreground">Trip Details</h3>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div className="pt-4 border-t border-border/50 mt-6">
                        ...
                        <Button type="submit" size="lg" className="w-full text-base font-bold uppercase tracking-wide h-12" disabled={isSubmitting || items.length === 0}>
...
function TripItemCard({ item, onRemove }: { item: TripItem; onRemove: () => void }) {
    // Helper to get image if available in data
    const image = item.data?.image || item.data?.airlineLogo || "/images/placeholder.jpg"; // Fallback
    ...
    return (
        <motion.div 
            ...
            className="flex flex-col sm:flex-row gap-4 bg-card border border-border/50 rounded-xl p-4 shadow-sm group hover:border-primary/20 transition-colors"
        >
            ...
            <div className="flex-1 flex flex-col justify-between">
                <div>
                    <div className="flex items-start justify-between gap-2">
                         <div>
                            <span className="text-[10px] uppercase tracking-widest text-primary font-medium">{item.type}</span>
                            <h4 className="font-medium text-lg leading-tight">{item.title}</h4>
                         </div>
                         {item.price ? (
                             <span className="font-mono text-sm font-medium whitespace-nowrap">${item.price}</span>
                         ) : null}
                    </div>
                    {item.description && (
            ...
            <div className="flex flex-row sm:flex-col justify-end gap-2 border-t sm:border-t-0 sm:border-l border-border/50 pt-3 sm:pt-0 sm:pl-3">
                <Button variant="ghost" size="icon" className="h-8 w-8 text-muted-foreground hover:text-destructive hover:bg-destructive/10" onClick={onRemove}>
